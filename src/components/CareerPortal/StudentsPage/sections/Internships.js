...
const Internships = () => {
  return (
    <section id="internships" className="py-16 bg-gradient-to-br from-white via-gray-50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-4xl font-bold text-center text-[#525599] mb-14">
        ...
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
          {internshipData.map((intern) => (
            <div
              key={intern.id}
              className="bg-white rounded-2xl border border-gray-200 shadow-md hover:shadow-lg transition-shadow p-6"
            >
              ...
              <div className="text-sm text-gray-500 space-y-1 mb-4">
                ...
                <p><span className="font-medium text-gray-700">🕒 Duration:</span> {intern.duration}</p>
                <p><span className="font-medium text-gray-700">💸 Stipend:</span> {intern.stipend}</p>
              </div>

              <p className="text-gray-700 text-sm leading-relaxed">{intern.description}</p>

              <button className="mt-5 inline-block px-4 py-2 bg-[#525599] text-white text-sm rounded-md hover:bg-[#41447d] transition-colors">
                Apply Now
              </button>
            </div>
          ))}
